...
public class RPGM_XP_Utility  {
        ...
        Texture2D[] top_left_mini_tiles, Texture2D[] top_right_mini_tiles, Dictionary<byte, int> rule_tiles)
    {
        ...
        foreach (var comb in Tiles_Utility.ByteCombination())
        {
            ...
            int bl = Select_Mini_Tile(comb, 5, 8, 7, 2, 4);
            int br = Select_Mini_Tile(comb, 7, 6, 4, 5, 2);
            int tr = Select_Mini_Tile(comb, 4, 1, 2, 7, 5);
            int tl = Select_Mini_Tile(comb, 2, 3, 5, 4, 7);
        ...
        out Texture2D[] top_left_mini_tiles, out Texture2D[] top_right_mini_tiles)
    {
        ...
        for(int i = 0; i < 6; i += 2)
        {
            for(int j = 0; j < 6; j += 2)
            {
                bottom_left_mini_tiles[cc++] = rawPieces[7-i, j];
            }
        }
        bottom_left_mini_tiles[cc++] = rawPieces[1, 4];
        ...
        for (int i = 0; i < 6; i += 2)
        {
            for (int j = 0; j < 6; j += 2)
            {
                bottom_right_mini_tiles[cc++] = rawPieces[7 - j, 5-i];
            }
        }
        bottom_right_mini_tiles[cc++] = rawPieces[1, 5];
        ...
        for (int i = 2; i < 8; i += 2)
        {
            for (int j = 0; j < 6; j += 2)
            {
                top_right_mini_tiles[cc++] = rawPieces[i, 5-j];
            }
        }
        top_right_mini_tiles[cc++] = rawPieces[0, 5];
        ...
        for (int i = 0; i < 6; i += 2)
        {
            for (int j = 2; j < 8; j += 2)
            {
                top_left_mini_tiles[cc++] = rawPieces[j, i];
            }
        }
        top_left_mini_tiles[cc++] = rawPieces[0, 4];
    }
}
